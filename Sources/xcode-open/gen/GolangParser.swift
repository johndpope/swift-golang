...
open class GolangParser: Parser {
	...
	func getATN() -> ATN { return GolangParser._ATN }
	     ...
	     * contains a line terminator.
	     */
	    private func lineTerminatorAhead()->Bool {
	        // Get the token ahead of the current index.
            
            do {
                var possibleIndexEosToken = try self.getCurrentToken().getTokenIndex() - 1;
                var ahead = try _input.get(possibleIndexEosToken);
                if (ahead.getChannel() != Lexer.HIDDEN) {
                    // We're only interested in tokens on the HIDDEN channel.
                    return false;
                }
                
                if (ahead.getType() == Tokens.TERMINATOR.rawValue) {
                    // There is definitely a line terminator ahead.
                    return true;
                }
                
                if (ahead.getType() == Tokens.WS.rawValue) {
                ...
                var  text = ahead.getText();
                var  type = ahead.getType();
                
                // Check if the token is, or contains a line terminator.
                return (type == Tokens.COMMENT.rawValue && ((text?.contains("\r"))! || (text?.contains("\n"))!)) ||
                    (type == Tokens.TERMINATOR.rawValue);
	...
	private func eos_sempred(_ _localctx: EosContext!,  _ predIndex: Int) throws -> Bool {
        ...
        case 7:return try _input.LT(1)?.getText() == "}"
		    default: return true
		}
	}
